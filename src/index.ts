...
export function decode(buf: Buffer, position: number = 0): ZoneInfoFile {
    ...
    const info: ZoneInfoFile = {
        magic: buf.toString(undefined, position + 0, position + 4),
        // @ts-ignore
        version: buf.toString(undefined, position + 4, position + 5),
        ttisgmtcnt: int(buf, position + 20),
        ttisstdcnt: int(buf, position + 24),
        leapcnt: int(buf, position + 28),
        timecnt: int(buf, position + 32),
        typecnt: int(buf, position + 36),
        charcnt: int(buf, position + 40),
        _end: 0,
        abbrevs: '',
        leaps: [],
        timezone: '',
